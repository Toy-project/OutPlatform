...
class SnippetEditPopupName extends React.Component {
  constructor(props) {
    super(props);

    this.state = {
      club_name : {
        value : this.props.club_name,
        err_msg : '변경할 동아리 이름을 입력해주세요.',
        err : null,
      }
    };
  ...
  handleChange(e) {
    ...
    let err_msg = '변경할 동아리 이름을 입력해주세요.';
    let err = null;

    if(target.id === 'club_name') {
      if(!Common.isEmpty(value)) {
        if(!RegisterHelper.isNameAvailable(value)) {
          err_msg = '동아리 이름은 2글자 이상, 10글자 미만으로 해주세요!';
          err = true;
        } else {
          err_msg = '등록가능한 동아리 이름입니다.';
          err = false;
        }
      }
    }
  ...
  handleBlur(e) {
    ...
    if(target.id === 'club_name') {
      if(target.value === this.props.club_name) {
        err_msg = '현재와 동일한 동아리 이름입니다.';
        err = true;
      } else if(!RegisterHelper.isNameAvailable(target.value)) {
        err_msg = '동아리 이름은 2글자 이상, 10글자 미만으로 해주세요!';
        err = true;
      } else {
  ...
  handleEmptyValue() {
    if(Common.isEmpty(this.state.club_name.value)) this.refs.club_name.focus();
    else {
      return true;
    }
  ...
  render() {
    ...
    const msg = () => {
      ...
      return this.state.club_name.err_msg;
    }
    return (
      <div className='popup_container'>
        <div className='snippet-edit-popup-container'>
          ...
          <div className='info-title'>
            <h1>동아리 이름 수정</h1>
            <label className={errorClassName(this.state.club_name)} htmlFor='club_name'>
              {msg()}
            </label>
          </div>
          <div className='info-input'>
            <input type="text" id='club_name' ref='club_name' onBlur={this.handleBlur} onChange={this.handleChange} defaultValue={this.props.club_name}/>
          </div>
...
SnippetEditPopupName.propTypes = {
};
...
export default connect(mapStateToProps, mapDispatchToProps)(SnippetEditPopupName);
