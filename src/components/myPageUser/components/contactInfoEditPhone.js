...
class ContactInfoEditPhone extends React.Component {
  ...
  handleSubmit(e) {
    ...
    if(isValid) {
      if(this.props.phone === this.state.phone.value) {
        this.setState({
          phone : {
            ...this.state.phone,
            err_msg : '현재와 같은 연락처입니다.',
            err: true,
          }
        });
  ...
  render() {
    ...
    const msg = () => {
      ...
      if(this.props.club.isLoading || this.props.member.isLoading) {
        return '잠시만 기달려주세요.';
      } else {
      ...
      <div className='popup_container'>
        <div className='contact-info-edit-phone'>
          ...
          <div className='info-title'>
            <h1>연락처 수정</h1>
            <label className={errorClassName(this.state.phone)} htmlFor='phone'>
              {msg()}
            </label>
          </div>
          {phone}
